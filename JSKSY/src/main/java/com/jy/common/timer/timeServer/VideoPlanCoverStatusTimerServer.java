 ...
 *
 */
public class VideoPlanCoverStatusTimerServer {

	public ApplicationContext context;

	public long dateOffset;

	public VideoPlanCoverStatusTimerServer(ApplicationContext cont) {
		this.context = cont;
		this.dateOffset = 1000 * 60 * 10; // 10分钟
	}

	public void start() {
		Timer timer = new Timer("setVideoPlanStatus");
		timer.schedule(new setVideoPlanStatus(), 1000 * 5, dateOffset);
	}

	class setVideoPlanStatus extends TimerTask {

		@Override
		public void run() {
			System.out.println("***************VideoPlanCoverStatusTimerServer-开始***************");
			...
			if(null != lists && lists.size() > 0) {
				for(Exam e : lists) {
					...
					for(ExamSeason ls : list) {
						...
						Filter.put("STATUSTYPE", 1);	//1-考前，2-考后
			...
			System.out.println("***************VideoPlanCoverStatusTimerServer-结束***************");
		}
