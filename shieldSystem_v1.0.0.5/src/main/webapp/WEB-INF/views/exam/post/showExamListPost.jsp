			...
			<tr>
				...
				<td class="status" data-status="${e.status}">
                    <c:if test="${e.status==1 }">
                        <%--<a onclick="startThread('${e.id }',2)">启用</a>--%>启用
                    </c:if>
                    <c:if test="${e.status==2 }">
                        <%--<a onclick="startThread('${e.id }',1)">禁用</a>--%>禁用
                    </c:if>
                </td>
				<td>
					<input type="hidden" class="examTypeId" value="${e.examTypeId }">
                    <img title="查看" onclick="previewExam(this)" class="tdImg" src="${basePath }/static/jy/img/pcImg/preview.png"/>
