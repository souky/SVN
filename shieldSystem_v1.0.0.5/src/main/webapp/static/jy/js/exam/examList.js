var pageSize = 10;var switchs = 0;
 $(function(){
	 ...
	 $.fn.datetimepicker.defaults = {
         ...
         minDate: new Date(),
	     //选择板所在输入框位置
	     pickerPosition: "bottem-left",
         clearBtn: true
	 };
     $("#startTimeSearch").datetimepicker();
     $("#endTimeSearch").datetimepicker();
     $(".datetimepicker").mouseenter(function(){
...
function exportExams(){
 ...
 function newExam(){
	 ...
	 $("#examStartTime").datetimepicker({
         ...
         $("#examEndTime").datetimepicker('setStartDate',startTime);
     });
     $("#examEndTime").datetimepicker({
         ...
         $("#examStartTime").datetimepicker('setEndDate',endTime);
     });
...
//编号自动填写
 ...
 function deletExam(obj){
     ...
     if(curTime >= startTime && curTime <= endTime){
         JRYPopup.Jalert("只可以删除考试计划未开始和已结束的考试计划");
         return;
     }
 ...
 function examSave(){
     ...
     if(examName == undefined || examName == ''){
         $("#examName").closest(".Jcustom_content_items").addClass("haserror");
         $("#examName").parent().find(".error_text").html("考试名称不能为空");
         return;
     }
     if(examName.length > 30){
         $("#examName").closest(".Jcustom_content_items").addClass("haserror");
         $("#examName").parent().find(".error_text").html("考试名称长度最大30字符");
         return;
     }
	...
	if(reg.test(examTypeId)){
	 ...
	 $.ajax({
		 ...
		 data:$.param({"examNo": examNo, "status": $("#status").val()}) + "&" + data,
		 type:'post',
		 success:function(res){
			 if(res.code=='10000'){
				 var pageNow = $("#pageCss").find(".pageNow").val();
...
function previewExam(obj){
    ...
    JRYPopup.Jcustom("newExam");
    $(".Jcustom_title").html("预览考试信息");
    $("#examId").val(examId);
    $("#examNo").val(examNo);
    $("#examName").val(examName);
    $("#examName").attr("readonly", "readonly").attr("disabled", "disabled");
    $("#examStartTime").val(examStartTime);
    $("#examStartTime").attr("readonly", "readonly").attr("disabled", "disabled");
    $("#examEndTime").val(examEndTime);
    $("#examEndTime").attr("readonly", "readonly").attr("disabled", "disabled");
    $("#status").val(status);
    $("#status").attr("readonly", "readonly").attr("disabled", "disabled");
    $("#examTypeId").val(examTypeId);
    $("#examTypeId").attr("readonly", "readonly").attr("disabled", "disabled");
    $("#exSub").hide();
    $(".exsubInfo").show();
    $(".exSubTableRead").show();
    $(".btnsExsub").hide();
...
function initexsubRead(){
    ...
    $.ajax({
        ...
        type:'post',
        success:function(res){
            var html = '<tr><th>场次序号</th><th>场次名称</th><th>开始时间</th><th>结束时间</th></tr>';
            var list = res.result;
            if(list.length==0){
                html+='<tr><td colspan="5">没有场次信息</td></tr>';
            }else{
            ...
            $(".exSubTableRead").find("table").html(html);
        },error:function(res){
        }
    })
}
 ...
 function initTable(pageNum,pageSize){
     ...
     var examNo = $('#examNoSearch').val();
     var examName = $('#examNameSearch').val();
     ...
     if(startTime != "" && startTime != undefined && endTime != "" && endTime != undefined && (date1 >= date2)){
         JRYPopup.Jalert("开始时间应该小于结束时间");
         return;
     }
...
$(document).bind("click",function(e){
    var _con = $('.Jalert_btn');   // 设置目标区域
    console.log(_con.length)
    if(_con.length > 0){
    	var s = e.target.getAttribute("onclick");
    	if(s != "saveExsub(this)"&&s !="edit(this)"){
    		JRYPopup.JalertClose();
    	}
    }
