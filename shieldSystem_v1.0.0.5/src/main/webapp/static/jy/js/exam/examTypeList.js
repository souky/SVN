...
function newExamType(){
    ...
    $('#examTypeNo').css("readonly",false); $('#examTypeNo').css("disabled",false);
...
function typeSaveOrEdit(){
    ...
    if(examTypeName.length > 30){
        $("#examTypeName").closest(".Jcustom_content_items").addClass("haserror");
        $("#examTypeName").parent().find(".error_text").html("考试类型名称长度最大30字符");
        return;
    }

	$.ajax({
		url:basepaths+'examtype/examTypeSaveOrUpdata',
		data:{'examTypeNo': examTypeNo, 'examTypeName': examTypeName, 'id': $("#id").val()},
		type:'post',
		success:function(res){
			if(res.code=='10000'){
				var pageNow = $("#pageCss").find(".pageNow").val();
...
function editType(obj){
    ...
    $('#examTypeNo').attr("readonly", true); $('#examTypeNo').attr("disabled", true);
}
//删除
