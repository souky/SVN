...
public class ExamController {
	...
	public ModelAndView getAllExamList(@RequestParam int pageNum,@RequestParam int pageSize,
        ...
        String begindateString = request.getParameter("startTime");
        String enddateString = request.getParameter("endTime");

        Map<String, Object> filter = new HashMap<>();
        if(StringUtils.isNoneBlank(examName)){
            filter.put("examName", examName.trim());
        }
        if(StringUtils.isNoneBlank(examNo)){
            filter.put("examNo", examNo.trim());
        }
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        if(StringUtils.isNoneBlank(begindateString)){
            try{
                Date startTime = sdf.parse(begindateString);
                filter.put("startTime", startTime);
            }catch (ParseException e){
                LOGGER.debug("开始时间转换异常" + e.getMessage());
            }
        }
        if(StringUtils.isNoneBlank(enddateString)){
            try{
                Date endTime = sdf.parse(enddateString);
                filter.put("endTime", endTime);
            }catch (ParseException e){
                LOGGER.debug("结束时间转换异常" + e.getMessage());
            }
        }

		ModelAndView mv = new ModelAndView();
		PageHelper.startPage(pageNum, pageSize);
    	PageInfo<Exam> exam = new PageInfo<Exam>(examService.queryExamFilter(filter));
	...
	public AsyncResponseData.ResultData saveOrUpdateExam(HttpServletRequest request){
		...
		Exam ex = examService.queryExamByExamName(examName.trim());
		if(ex != null && !ex.getId().equals(id)){
			return AsyncResponseData.getSuccess().asParamError("考试计划名称不能重复");
		}

		Map<String, Object> filter = new HashMap<String, Object>();
		//2018-4-8 09:25:05 去掉一个考试类型只能有一个考试计划验证
		...
		try {
			...
			if(begindate.before(new Date())){
                return AsyncResponseData.getSuccess().asParamError("开始时间必须大于当前时间");
            }
